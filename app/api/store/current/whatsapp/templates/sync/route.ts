...
export async function POST(req: Request) {
  try {
    ...
    const result = await syncTemplateStatusesForStore(authResult.storeId)
    ...
    console.error("[POST /api/store/current/whatsapp/templates/sync]", error)
    return serverError(
      "Não foi possível sincronizar os templates WhatsApp."
    )
  }
}
